  ...
  async startServer() {
    ...
    const server = http.createServer(async (req, res) => {
      try {
        ...
        const sessionHeader = this.mcpSessionId || req.headers['mcp-session-id'] || req.headers['Mcp-Session-Id'];

        // JSON-RPC notifications do not receive a response body.
        if (response === null) {
          const headers = sessionHeader ? { 'Mcp-Session-Id': String(sessionHeader) } : {};
        ...
        if (sessionHeader) headers['Mcp-Session-Id'] = String(sessionHeader);
  ...
  async handleJsonRpc(message) {
    ...
    try {
      ...
      if (method === 'initialize') {
        ...
        const protocolVersion = clientProtocolVersion || DEFAULT_PROTOCOL_VERSION;
        this.mcpSessionId = crypto.randomBytes(16).toString('hex');
...
class ObsidianMcpSettingTab extends PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl('h2', { text: 'MCP-Util Settings' });
    ...
    const codexBearer = authToken ? `Bearer ${authToken}` : 'Bearer (token value)';
    const codexConfig = `[mcp_servers.obsidian]
url = "${mcpUrl}"

[mcp_servers.obsidian.http_headers]
Authorization = "${codexBearer}"`;
